...
void leadertoworld2D(geometry_msgs::Point &follower_goal, turtlesim::Pose &leader)
{
  ...
  follower_goal.x = (x1*cos(-leader.theta) + y1*sin(-leader.theta)) + leader.x;
  follower_goal.y = (-x1*sin(-leader.theta) + y1*cos(-leader.theta)) + leader.y;
} 
...
void worldtobody2D(float &x, float &y, float theta)
{
  ...
  x = x1_*cos(theta) + y1_*sin(theta);
  y = -x1_*sin(theta) + y1_*cos(theta);
}
...
void Positioncontrol(geometry_msgs::Point &goal, turtlesim::Pose &follower, geometry_msgs::Twist &vel_msg)
{
	...
	vel_msg.linear.x = 1*error_norm;
	vel_msg.angular.z = 2*error_theta;

	if (0 < abs(error_norm) && abs(error_norm) < 0.1)
	{
		vel_msg.linear.x = 0;
	}

	if (0 < abs(error_theta) && abs(error_theta) < 0.01)
	{
		vel_msg.angular.z = 0;
	}
}
